"""
MC-finetuned → Open QA generalisation evaluation for LLaMA-3.2-3B-Instruct.

Loads the MC-trained QLoRA adapter from sft_llama3p2_3b_qlora_safe_resume2,
...
Critical fix vs. the original generalization_pipeline.py:
  The base model MUST be loaded with the same 4-bit NF4 quantisation config
  that was used during training.  Loading in full precision (bf16/fp16)
  creates a distribution mismatch with the adapter weights and produces
  0.00 accuracy.
"""
...
ADAPTER_PATH = "sft_llama3p2_3b_qlora_safe_resume2"
INPUT_JSON = "combined_QA_generation2.json"
OUTPUT_DIR = "generalization_results_llama3p2_3b"
