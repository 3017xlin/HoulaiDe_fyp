"""
MC QLoRA fine-tuning + post-training full-eval script for LLaMA-3.1-8B-Instruct.
...
class CFG:
    # ---- Model ----
    # Gated model: need HF token with Llama-3.1 access accepted on the Hub.
    model_name: str = "meta-llama/Llama-3.1-8B-Instruct"

    train_csv: str = "prepared_data/train_split.csv"
    eval_csv: str = "prepared_data/eval_split.csv"

    output_dir: str = "sft_llama3p1_8b_qlora_safe_resume2"

    max_length: int = 384
    lr: float = 2e-4
    epochs: float = 2.0

    # 8B QLoRA on a single 24GB / 40GB GPU:
    # keep micro-batch at 1 but raise grad-accum to 16 so the effective batch
    # (16) is a bit bigger and gives a more stable update on the larger model.
...
def main():
    ...
    result_path = "mc_train_and_large_eval_results_llama3p1_8b.txt"
