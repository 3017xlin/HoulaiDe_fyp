"""
MC QLoRA fine-tuning + post-training full-eval script for LLaMA-3.2-3B-Instruct.

This script is a direct sibling of train_and_full_eval_mc.py (Qwen2.5-1.5B-Instruct).
The training pipeline, prompt construction, tokenisation, LoRA hyper-parameters,
evaluation loop and metrics are kept identical to the Qwen baseline so that the
four-model comparison (Qwen2.5-1.5B / Llama-3.2-3B / Phi-3.5-mini / Llama-3.1-8B)
only differs in the base model.

Note:
  * meta-llama/Llama-3.2-3B-Instruct is a gated model. You must accept the
    license on the Hugging Face Hub and export HF_TOKEN before running, e.g.
        export HF_TOKEN=hf_xxxxxxxxxxxxxxxxxxxx
    or pre-run `huggingface-cli login`.
"""
...
# =========================
# Config (LLaMA-3.2-3B-Instruct)
# =========================
@dataclass
class CFG:
    # ---- Model ----
    # Gated model: need HF token with Llama-3.2 access accepted on the Hub.
    model_name: str = "meta-llama/Llama-3.2-3B-Instruct"

    train_csv: str = "prepared_data/train_split.csv"
    eval_csv: str = "prepared_data/eval_split.csv"

    output_dir: str = "sft_llama3p2_3b_qlora_safe_resume2"

    max_length: int = 384
    lr: float = 2e-4
    epochs: float = 2.0

    # 3B QLoRA fits on 1×24GB at micro-batch 1 with grad-accum 8
...
def make_model_and_tokenizer(cfg: CFG):
    ...
    # Llama-3.x ships with no pad_token; reuse eos.
    ...
    # LLaMA-3.x attention uses q_proj / k_proj / v_proj / o_proj — same as Qwen2.5.
...
def main():
    ...
    result_path = "mc_train_and_large_eval_results_llama3p2_3b.txt"
